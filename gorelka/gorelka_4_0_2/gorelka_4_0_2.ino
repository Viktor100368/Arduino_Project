...
byte *p_var_menu;
char* p_menu;
int datchik_plameni=11;    //датчик наличия пламени
int zapusk=10;    //пин управления катушкой зажигания
int knop_pin=12;    //управление кнопкой энкодера
int urowen_up=6;    //верхний уровень масла
int urowen_down=7;   //нижний уровень масла
int podkach_pin=18;   //управление насосом подкачки
int cirkulacii_pin=19;  //циркуляионный насос
int nagrew_pin=16;      //управление теном нагрева масла
int vozdux_pin=17;      //управление подачей воздуха
...
delay(300);  //дребезг
 ...
 while(1)   //цикл управления работой горелки
  ...
  if(count_lcd >=2)        //прошло 2 сек 
    ...
    if((analogRead(urowen_up)<300)&&(analogRead(urowen_down)<300))   //бак пустой
          {
            digitalWrite(podkach_pin,LOW);              //включаем подкачку
            urowen=false;                   //олускаем флаг уровня, наливаемся
//            Serial.print("uruwen");
//            Serial.println(urowen);
           }
           else if ((analogRead(urowen_up)>700)&& (analogRead(urowen_down)>700)) //масло налили
           {
            digitalWrite(podkach_pin,HIGH);                            //выключаем подкачку
            urowen=true;                           //поднимаем флаг, можно работать
//            Serial.print("uruwen");
//            Serial.println(urowen);
           }
           else if ((analogRead(urowen_up)<300)&& (analogRead(urowen_down)>700)) //если маслао в пределах и пламя есть
...
//      if ((t_oil < EEPROM.read(Oil_min))&&!job)   //масло остыло
      ...
      {
         digitalWrite(nagrew_pin,LOW); // включаем нагрев
       ...
       {
         digitalWrite(nagrew_pin,HIGH);// выключаем нагрев
         job=true;                   //можно работать
       }
   }   
     else
    { 
        digitalWrite(nagrew_pin,HIGH);// масло в диапазоне
        job=true;   //можно работать
    }
    
    if((t_oil<75)&&rabota_g)
  {
   digitalWrite(vozdux_pin,HIGH); //при работе масло остыло до 70, гасим горелку
   digitalWrite(cirkulacii_pin,HIGH); //выключаем циркуляцию
    job=false;  //стоим, греемся
  }
  
     //..........................................................запуск горелки
      if((t_water<=EEPROM.read(Water_min))&&job && !rabota_g&& urowen)//если вода остыла, масло горячее и пламени нет и уровень в норме
       {
           digitalWrite(cirkulacii_pin,LOW);    //включаем циркуляцию, ждем , что бы масло прокачалось
           if(t_water<40) zaderghka=60000;// вода холоднее 40, прокачмваем масло 90 сек
              else
                   zaderghka=30000; //иначе, прокачиваем масло 30 сек 
           ...
           while(1)
            {
               ...
               if((wremja_2-wremja_3) >=5000) //прошло 5 сек, идем смотреть температуру
              {
                 izmerenieTemper(p_t_oil,p_t_water);    //уходим мерить температуру
                 lcd_out();             // уходим писать лсд
                if (t_oil < (EEPROM.read(Oil_max)-5))   //масло остыло
                     digitalWrite(nagrew_pin,LOW); // включаем нагрев) 
                    else if(t_oil >(EEPROM.read(Oil_max))//масло нагрелось
                       digitalWrite(nagrew_pin,HIGH);//Масло нагрелось
                 wremja_3=wremja_2;
              }
...
//               delay(1000);
          ...
          digitalWrite(zapusk,HIGH); // выключаем искру
       }
...
//      }
  ...
  if (digitalRead(datchik_plameni)==HIGH)   //если пламени нет
     {
        digitalWrite(cirkulacii_pin,HIGH);      //выключаем циркуляцию
        digitalWrite(vozdux_pin,HIGH);      //выключаем воздух
...
boolean Zapusk(boolean *p)// запуск горелки
{
  ...
  do
  {
     i++;
      ...
      digitalWrite(zapusk,LOW);    // подаем искру
      delay(500);
      digitalWrite(vozdux_pin,LOW);// подаем воздух
      delay(2500);
        ...
        goto label2;   //есть работа 
         ...
         {
           digitalWrite(vozdux_pin,HIGH);
           digitalWrite(zapusk,HIGH);// иначе все выключаем, работы нет
